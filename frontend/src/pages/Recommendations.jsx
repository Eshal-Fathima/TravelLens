...
// ─── Static travel tips ───────────────────────────────────────────────────────
const STATIC_TRAVEL_TIPS = [
  { icon: '📸', title: 'Golden Hour Shots', description: 'The best travel photos happen 30 minutes after sunrise and before sunset.' },
  { icon: '🗺️', title: 'Download Offline Maps', description: 'Always save offline maps before heading to remote or rural areas.' },
  { icon: '💳', title: 'Carry Local Currency', description: 'Many local vendors and temples accept cash only — always carry some.' },
  { icon: '🎒', title: 'Travel Insurance', description: 'Even domestic trips benefit from basic travel insurance for cancellations.' },
]
...
export default function Recommendations() {
  ...
  // ── Loading ──
  if (loading) return (
    <div style={{ minHeight: '100vh', background: themes[isDark ? 'dark' : 'light'].bg, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 16, fontFamily: 'Manrope, sans-serif' }}>
      <div style={{ display: 'flex', gap: 6 }}>
        {[0, 1, 2].map(i => (
          <div key={i} style={{
            width: 8, height: 8, borderRadius: '50%',
            background: themes[isDark ? 'dark' : 'light'].primary,
            animation: `rec-dot 1.2s ${i * 0.2}s ease-in-out infinite`,
          }} />
        ))}
      </div>
      <p style={{ fontSize: 13, color: themes[isDark ? 'dark' : 'light'].textMuted, margin: 0 }}>Our RAG model is thinking…</p>
      <style>{`@keyframes rec-dot{0%,80%,100%{transform:scale(0.6);opacity:0.3}40%{transform:scale(1);opacity:1}}`}</style>
    </div>
  )

  // ── Error ──
  if (error) return (
    <div style={{ minHeight: '100vh', background: themes[isDark ? 'dark' : 'light'].bg, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'Manrope, sans-serif' }}>
    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .rec-tip-row { transition: background 0.18s; border-radius: 12px; }
      ...
      <div style={{ minHeight: '100vh', background: t.bg, fontFamily: 'Manrope, sans-serif', transition: 'background 0.3s' }}>
        ...
        <div style={{ position: 'relative', zIndex: 1, maxWidth: 1080, margin: '0 auto', padding: '0 24px 100px' }}>
          ...
          <header style={{ margin: '48px 0 40px', animation: 'rec-fade 0.45s ease both' }}>
            ...
            <p style={{ fontSize: 16, color: t.textMuted, maxWidth: 560, lineHeight: 1.7, margin: '0 0 16px' }}>
              Curated expeditions and hidden gems tailored to your travel personality. Your next narrative starts here.
            </p>
            <div style={{
              display: 'inline-flex', alignItems: 'center', gap: 8,
              padding: '6px 14px', borderRadius: 999,
              background: t.primaryFaded, border: `1px solid ${t.primary}30`,
              fontSize: 11, fontWeight: 700, color: t.primary, letterSpacing: '0.5px',
            }}>
              <SparkleIcon />
              Powered by your travel history · Retrieval-based AI
            </div>
          </header>
          ...
          {!recs ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 56 }}>
              ...
              {recs.destinations?.length > 0 && (
                <section style={{ animation: 'rec-fade 0.45s 0.08s ease both' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
                    ...
                    <span style={{ fontSize: 11, fontWeight: 700, letterSpacing: '1.5px', textTransform: 'uppercase', color: t.primary }}>
                      {recs.destinations.length} Places
                    </span>
                  </div>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 16, marginBottom: 16 }}>
                    {recs.destinations[0] && (
                      <div className="rec-hover rec-img-card" style={{
                        ...
                        minHeight: 380, background: t.heroBg,
                        ...
                        <div style={{ position: 'absolute', top: 20, left: 20, right: 20, display: 'flex', justifyContent: 'space-between', alignItems: 'center', zIndex: 2 }}>
                          ...
                          }}>{recs.destinations[0].category}</span>
                          <button onClick={() => toggleSave('dest-0')} style={{
                        ...
                        <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: '32px', zIndex: 2 }}>
                          ...
                          <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.8)', lineHeight: 1.7, margin: '0 0 20px', maxWidth: 400 }}>
                    ...
                    {recs.destinations[1] && (
                      <div className="rec-hover" style={{
                        ...
                        <div>
                          ...
                          }}>{recs.destinations[1].category}</span>
                        ...
                        <div style={{ display: 'flex', gap: 8, marginTop: 24 }}>
                          <button onClick={() => toggleSave('dest-1')} style={{
                            display: 'flex', alignItems: 'center', gap: 6,
                            padding: '8px 16px', borderRadius: 10,
                            background: saved.has('dest-1') ? 'rgba(239,68,68,0.1)' : t.primaryFaded,
                            border: `1px solid ${saved.has('dest-1') ? 'rgba(239,68,68,0.3)' : t.border}`,
                            color: saved.has('dest-1') ? '#ef4444' : t.primary,
                            fontSize: 12, fontWeight: 700, cursor: 'pointer',
                            fontFamily: 'Manrope, sans-serif',
                          }}>
                            <HeartIcon filled={saved.has('dest-1')} />
                            {saved.has('dest-1') ? 'Saved' : 'Save'}
                          </button>
                        </div>
                      </div>
                    )}
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 48, animation: 'rec-fade 0.45s 0.16s ease both' }}>
                ...
                {recs.places?.length > 0 && (
                  <section>
                    <h2 style={{
                      ...
                      fontFamily: 'Manrope, sans-serif',
                      borderLeft: `4px solid ${t.primary}`, paddingLeft: 14,
                    }}>Places to Visit</h2>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                      {recs.places.map((place, i) => (
                        <div key={i} className="rec-tip-row" style={{ display: 'flex', gap: 18, alignItems: 'flex-start', padding: '16px' }}>
                          <div style={{
                ...
                <section>
                  <h2 style={{
                    ...
                    letterSpacing: '-0.5px', margin: '0 0 24px',
                    fontFamily: 'Manrope, sans-serif',
                    borderLeft: `4px solid ${t.accent}`, paddingLeft: 14,
                  }}>Expert Insights</h2>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
                    {STATIC_TRAVEL_TIPS.map((tip, i) => (
                      <div key={i} className="rec-hover" style={{
                        background: t.travelTipBg, border: `1px solid ${t.border}`,
              ...
              {recs.budget_tips?.length > 0 && (
                <section style={{
                  ...
                  <div style={{ textAlign: 'center', marginBottom: 32 }}>
                    <h2 style={{ fontSize: 32, fontWeight: 700, fontFamily: 'Lora, serif', color: t.primary, margin: '0 0 8px' }}>
                      Budget Optimization
                    </h2>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(260px,1fr))', gap: 16 }}>
                    {recs.budget_tips.map((tip, i) => (
                      <div key={i} className="rec-hover" style={{
                        ...
                        background: t.tipBg, border: `1px solid ${t.tipBorder}`,
                        position: 'relative', overflow: 'hidden',
                      }}>
                        <div style={{ position: 'absolute', top: 8, right: 8, fontSize: 56, opacity: 0.08, pointerEvents: 'none' }}>💰</div>
                        <span style={{
                        ...
                        <p style={{ fontSize: 14, color: t.textPrimary, lineHeight: 1.6, margin: '0 0 12px', fontWeight: 600 }}>
                          {tip.tip}
                        </p>
                        {tip.savings && (
                          <span style={{
                      ...
                      return (
                        <div key={i} className="rec-hover" style={{
                          ...
                          <div style={{ position: 'relative', zIndex: 1 }}>
                            <h3 style={{ fontSize: 24, fontWeight: 700, fontFamily: 'Lora, serif', fontStyle: 'italic', color: '#fff', margin: '0 0 12px' }}>
                              {s.type}
                            </h3>
                            ...
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
                              ...
                              <button className="rec-btn" style={{
                                ...
                                fontFamily: 'Manrope, sans-serif', marginLeft: 'auto',
